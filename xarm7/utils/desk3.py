...
def create_scene():
    ...
    table_height = 0.37
    table_top = scene.add_entity(gs.morphs.Box(
        size=(1.0, 0.6, 0.02),  # 长宽高
        pos=(0.0, 0.0, table_height),  # 放置在 table_height=0.26 上方
        fixed = True,
    ))
    ...
    leg_offset_x = 1.0 / 2 - leg_width / 2
    leg_offset_y = 0.5 / 2 - leg_width / 2

    for dx in [-leg_offset_x, leg_offset_x]:
        for dy in [-leg_offset_y, leg_offset_y]:
            scene.add_entity(gs.gs.morphs.Box(
                size=(leg_width, leg_width, leg_height),
                pos=(dx, dy, leg_height / 2),
                fixed = True,
            ))
